 ...
 * @version 1.0
 */
public abstract class DrawOtherPcController extends DrawHighPcController {

	@Override
	public ModelAndView date(@PathVariable @DateTimeFormat(pattern = "yyyyMMdd") Date date) {
		ModelAndView mav = super.date(date);
		mav.setViewName("pc/high/other/" + DrawLotteryConstant.getLotteryKey(getLotteryCode()));
		return mav;
	}
