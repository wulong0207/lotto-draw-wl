 ...
 */
@Controller
@RequestMapping("/h5/jq4")
public class DrawSportJq4H5Controller extends BaseH5Controller {

	private static final Logger logger = LoggerFactory.getLogger(DrawSportJq4H5Controller.class);
	...
	public ModelAndView index() {
		...
		String url = drawCoreUrl + "draw/jq4/list";
		...
		if (result == null || result.isError() || result.getData() == null) {
			logger.error("查询四场进球开奖列表失败:{}", result.getMessage());
		...
		// 彩种信息
		DrawLotteryTypeBO lotteryType = drawLotteryService.findLotteryType(LotteryEnum.Lottery.JQ4.getName());
		ModelAndView mav = new ModelAndView();
		mav.addObject("lotteryType", lotteryType);
		mav.addObject("issueList", issueList);
		mav.addObject("lastIssue", lastIssue);
		mav.setViewName("h5/sport/jq4");
		return mav;
	}
	...
	public ModelAndView detail(@PathVariable("issueCode") String issueCode) {
		...
		String url = drawCoreUrl + "draw/jq4/detail";
		...
		if (result == null || result.isError() || result.getData() == null) {
			logger.error("查询四场进球开奖详情{}失败: {}", issueCode, result.getMessage());
			throw new ServiceRuntimeException(result);
		}
		// 彩种信息
		DrawLotteryTypeBO lotteryType = drawLotteryService.findLotteryType(LotteryEnum.Lottery.JQ4.getName());
		...
		mav.setViewName("h5/sport/jq4-detail");
		return mav;
	}
