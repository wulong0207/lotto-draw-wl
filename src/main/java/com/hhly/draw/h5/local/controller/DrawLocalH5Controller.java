 ...
 * @version 1.0
 */
public abstract class DrawLocalH5Controller extends BaseH5Controller {

	private static final Logger logger = LoggerFactory.getLogger(DrawLocalH5Controller.class);
	...
	public ModelAndView index() {
		...
		if (result == null || result.isError() || result.getData() == null) {
			logger.error("查询地方彩{}开奖列表失败:{}", getLotteryCode(), result.getMessage());
		...
		mav.setViewName("h5/local/" + DrawLotteryConstant.getLotteryKey(getLotteryCode()));
		return mav;
	}
	...
	public ModelAndView detail(@PathVariable("issueCode") String issueCode) {
		...
		if (result == null || result.isError() || result.getData() == null) {
			logger.error("查询地方开奖详情{}-{}失败: {}", getLotteryCode(), issueCode, result.getMessage());
			throw new ServiceRuntimeException(result);
		}
		...
		mav.setViewName("h5/local/" + DrawLotteryConstant.getLotteryKey(getLotteryCode()) + "-detail");
		return mav;
	}
