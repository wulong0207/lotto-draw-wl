 ...
 * @version 1.0
 */
public abstract class DrawOtherH5Controller extends DrawHighH5Controller {

	@Override
	public ModelAndView date(@PathVariable @DateTimeFormat(pattern = "yyyyMMdd") Date date) {
		ModelAndView mav = super.date(date);
		mav.setViewName("h5/high/other/" + DrawLotteryConstant.getLotteryKey(getLotteryCode()));
		return mav;
	}
	...
	public ModelAndView detail() {
		...
		mav.setViewName("h5/high/other/" + DrawLotteryConstant.getLotteryKey(getLotteryCode()) + "-detail");
		return mav;
	}
}
