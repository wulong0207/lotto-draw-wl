 ...
 * @version 1.0
 */
public abstract class DrawKl10H5Controller extends DrawHighH5Controller {

	@Override
	public ModelAndView date(@PathVariable @DateTimeFormat(pattern = "yyyyMMdd") Date date) {
		ModelAndView mav = super.date(date);
		mav.setViewName("h5/high/kl10/" + DrawLotteryConstant.getLotteryKey(getLotteryCode()));
		return mav;
	}
	...
	public ModelAndView detail() {
		...
		mav.setViewName("h5/high/kl10/" + DrawLotteryConstant.getLotteryKey(getLotteryCode()) + "-detail");
		return mav;
	}
}
